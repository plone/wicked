...
class WickedFilter(fapi.MacroSubstitutionFilter):
    ...
    def _filterCore(self, instance, chunk, **kwargs):
        """ Use the portal catalog to find a list of possible links. fiter by path, present by macro"""
        ...
        kwargs['links'] = [ (brain.getPath(), brain.getIcon) for brain in catalog(**scope) ]
        kwargs['chunk'] = chunk
